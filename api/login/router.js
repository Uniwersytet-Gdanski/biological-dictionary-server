...
const router = {
	[POST]: [
		...
		async (req, res, {yupJsonBody}) => {
			...
			return res.setStatusCode(204).setCookie("sessionToken", session.token, {...cookieOptions, httpOnly: true}).setCookie("sessionId", session.id, {...cookieOptions, httpOnly: false}).end();
		},
	],
};
